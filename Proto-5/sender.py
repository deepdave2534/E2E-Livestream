...
HMAC_SECRET_KEY = b'super_secure_key'  # Shared secret for HMAC authentication

def generate_hmac(tag, encrypted_frame):
    """ Generate HMAC for integrity verification """
    h = hmac.new(HMAC_SECRET_KEY, tag + encrypted_frame, hashlib.sha256)
    return h.digest()
        ...
        while streaming and cap.isOpened():
            ...
            for conn in list(clients):
                try:
                    ...
                    hmac_value = generate_hmac(tag, encrypted_frame)

                    data_packet = pickle.dumps((nonce, tag, encrypted_frame, frame_hash, hmac_value))
